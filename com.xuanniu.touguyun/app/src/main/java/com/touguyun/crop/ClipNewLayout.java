...
public class ClipNewLayout extends RelativeLayout implements OnTouchListener {
    ...
    private static final int SIZE_DEFAULT = 2048;		//手指点击时精度


    private ClipNewView mClipView;
    private ImageView mSouceImageView;

    private int mClipViewHeight = SIZE_DEFAULT;
    private int mClipViewWidth = SIZE_DEFAULT;
...
//    private float oldRotation = 0;
    ...
    private void init(Context context) {
        ...
        LayoutInflater.from(getContext()).inflate(R.layout.clip_picture_layout_new,
                this);
...
//    * @param bitmap
    ...
    * @param window
    */
    public void setSourceImage(Uri imageUri, Window window) {
        if (StringUtils.isEmpty(imageUri)){
            return;
        }
        mSouceBitmap = Images.decodeImageWithUri(getContext(), imageUri);
...
// --Commented out by Inspection START (15/7/9 上午11:51):
//    // 取旋转角度
...
//    }
// --Commented out by Inspection STOP (15/7/9 上午11:51)
...
//        Log.e(event.getX(0) + "", event.getY(0) + "");
    ...
    public Bitmap getBitmap() {
        ...
        if (rect == null || mSouceBitmap == null) {
            return null;
        }
        float scale = 1;
        int maxEdge = (int) Math.max(rect.width(), rect.height());
        if (maxEdge > SIZE_DEFAULT) {
            scale = (float) SIZE_DEFAULT / (float) maxEdge;
        }
        Matrix matrix = new Matrix();
        matrix.setRotate(getDegrees());
        matrix.postScale(scale, scale);
        return Bitmap.createBitmap(mSouceBitmap, (int) (rect.left),
